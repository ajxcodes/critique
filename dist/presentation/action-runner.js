...
const github_actions_adapter_1 = require("../infrastructure/github-actions-adapter");
...
async function runGitHubAction() {
    try {
        ...
        const apiKey = geminiKeyInput || (0, gemini_client_1.resolveGeminiApiKey)();
        if (!apiKey) {
            core.warning(constants_1.MSG_MISSING_API_KEY);
            return;
        }
        const githubToken = githubTokenInput || (0, gemini_client_1.resolveGitHubToken)();
        ...
        // 1. Extract Diff
        const diffProvider = new diff_provider_1.DiffProvider();
        const diffText = await diffProvider.getDiff({
            baseRef: `origin/${baseRef}`,
            maxChars: constants_1.AI_REVIEWER_MAX_PR_DIFF_CHARS
        });
        ...
        // 2. Resolve Standards
        const standardsResolver = new standards_resolver_1.StandardsResolver();
        ...
        if (githubToken && prNumber) {
            ghAdapter = new github_actions_adapter_1.GitHubActionsAdapter(githubToken);
        ...
        const prompt = (0, run_critique_1.buildCritiquePrompt)(diffText, standards.content, previousThreadsText);
        ...
        const report = ai_review_report_1.AiReviewReport.parse(queryResult.text, queryResult.modelUsed);
        ...
        if (ghAdapter && prNumber) {
            ...
            if (postInlineComments && report.findings.length > 0) {
                ...
                for (const finding of report.findings) {
                    const isDuplicate = unresolvedThreads.some((t) => t.path === finding.path && parseInt(t.line, 10) === finding.line);
                    if (isDuplicate) {
                        continue;
                    }
